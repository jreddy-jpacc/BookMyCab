...
namespace BookMyCab.WebAPI.Models.StorageService
{
    public class BookMyCabRepository
    {
        ...
        private CloudTable transactionTable;
        ...
        /// </summary>
        /// <returns></returns>
        private bool CreateCustomerTable()
        {
            bool status;
            try
            {
        ...
        public bool UpdateWallet(Wallet wallet)
        {
            try
            {
                ...
                string query = "RowKey eq '" + wallet.customerId + "'";
                bool status = false;
                ...
                if (custObj != null)
                {
                    ...
                    if (wallet.type.Equals("Debit"))
                    {
                        if (wallet.amount <= balance)
                        {
                            double offerAmount = 0;
                            offerAmount = (custObj.Offer * (wallet.amount / 10));
                            custObj.Balance -= (wallet.amount - offerAmount);
                            InsertEntity<Customer>(customerTable, custObj, false);
                            tranObj.TransactionDate = DateTime.Now;
                            tranObj.Type = "Debit";
                            tranObj.Amount = offerAmount;
                            tranObj.DriverId = wallet.driverId;
                            InsertEntity<Transaction>(transactionTable, tranObj, true);
                            status = true;
                        }
                    }
                    else if (wallet.type.Equals("Credit") && (wallet.amount >= 100 && wallet.amount <= 5000))
                    {
                        custObj.Balance += wallet.amount;
                        InsertEntity<Customer>(customerTable, custObj, false);
                        tranObj.TransactionDate = DateTime.Now;
                        tranObj.Type = "Credit";
                        tranObj.Amount = wallet.amount;
                        tranObj.DriverId = "0";
                        InsertEntity<Transaction>(transactionTable, tranObj, true);
                        status = true;
                    }
        ...
        /// <returns></returns>
        private List<T> RetrieveEntity<T>(CloudTable table, string query = null) where T : TableEntity, new()
        {
            try
            {
        ...
        private void InsertEntity<T>(CloudTable table, T entity, bool forInsert = true) where T : TableEntity, new()
        {
            try
            {
                if (forInsert)
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private bool DeleteEntity<T>(CloudTable table, T entity) where T : TableEntity, new()
        {
            try
            {
